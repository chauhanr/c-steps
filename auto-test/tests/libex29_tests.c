...
int check_function(const char *func_to_run, const char *data, int expected){
  ...
  check(func != NULL, "Did not find the %s function in library %s:%s", func_to_run, lib_file, dlerror());
  int rc = func(data); 
...
char *test_dlopen(){
 ...
 mu_assert(lib != NULL, "Failed to open library test."); 
 return NULL; 
}
...
char *test_failures(){
  mu_assert(check_function("failed_on_purpose", "Hello", 1), "failed on purpose should fail"); 
  return NULL; 
}
